...
- **[M26 進行中] 子切序 R1 → R4(R1.5 插站)**:R1 三模式框架+切區聚焦(✅)→ R1.5 框線補齊+日誌寬度 flex(✅;§6.1 完整框線網格是 M19–M22 未竟項,非拍板省略)→ R2 游標+捲動(component 長游標能力、方向鍵分區語意、游標態高亮、§6.12 cursor-follow 雙端記號、日誌 viewport)→ R3 modal 框架+計數 modal(model 持 modal 堆疊、80×22 通則+捲動標示、modal 態鍵表、F1/狀態列 Enter)→ R4 四張檢視 modal(顧客/行動/效果/卡牌,Enter 對游標項目)。
