...
- **M0–M25 已落地,M26(互動與 modal)進行中**:核心線(M0–M16)→ 事件流收口(M17–M18)→ TUI 被動觀看(M19–M22)→ 暫停機重構(M23)→ 日誌流換軌(M24)→ 速率(M25)。M26 子切 R1(三模式鍵盤框架+切區聚焦)已落地,R2 起未動(子切與拍板見接續待辦)。建置 / golangci-lint(0 issues)/ 測試全綠,cores 與 rules 覆蓋率 100%(app/rodi 除 TTY 組裝入口 Run 外 100%)。
...
- **[M26 進行中] 子切序 R1 → R4(R1.5 插站)**:R1 三模式框架+切區聚焦(✅)→ R1.5 框線補齊+日誌寬度 flex(✅;§6.1 完整框線網格是 M19–M22 未竟項,非拍板省略)→ R2 游標+捲動(✅;component 介面長 `View(game, width, focus)` + `Move(game, key)`、cursor-follow 窗格起點純由游標導出不另存狀態、`Item(game)` 延後到 R4 由 modal 消費者驅動)→ R3 modal 框架+計數 modal(model 持 modal 堆疊、80×22 通則+捲動標示、modal 態鍵表、F1/狀態列 Enter)→ R4 四張檢視 modal(顧客/行動/效果/卡牌,Enter 對游標項目;含 `Item` 介面)。
