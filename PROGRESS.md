...
- **全面統一 `Kind` 命名(比照 `exprs.tokenKind`)**:cores 六個列舉 `EventKind` / `PhaseKind` / `TriggerKind` / `EffectKind` / `TargetKind` / `TaskKind`;**常數去掉中間字**(如 `EventInstance` / `PhaseNone` / `EffectImmed` / `TargetNone` / `TaskSate`,非 `EventKindInstance`),**struct 列舉欄位用裸 `Kind`**(`EventData.Kind` / `Action.Kind`,比照 `token.kind`)。其中 `EffectKind` 以 `Kind` 後綴避開效果實例 struct `Effect` 撞名(效果實例維持 `cores.Effect`)。**生成碼欄位亦同步**:Effect 表(`gamedata/Effect.xlsx`)目標類型 / 觸發時機欄位改名 `TargetKind` / `TriggerKind`,`sheeter.Effect` 隨之生成同名欄位;**唯效果類型欄位字面名為 `Type`**(欄位本身即叫 Type),故 `Effect.Type` 維持 `Type`、不改。⚠️ **規格三書尚待同步**:`營業規格書` §二 詞彙表(目標類型 `TargetType` / 觸發時機 `TriggerType`)、`營業實作規格書` §二 prose、`營業顯示規格書` 效果欄位表仍是舊 `Type` 名;待確認詞彙決策(含 行動類型 `TaskType`、效果類型 `Type` 是否一併改)後再改 doc 並 rebuild。
