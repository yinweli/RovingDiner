...
func newLogPanel(sheet *sheeter.Sheeter) *logPanel {
	return &logPanel{journal: newJournal(sheet)}
}

// Append 收一筆事件(轉寫委派 journal)。
func (this *logPanel) Append(eventData cores.EventData) {
	this.journal.Append(eventData)
}

// View 渲染標題列 + 行歷史共 height 行: 行多取尾段釘最新(與步進同一時間軸, 下一行即下一步)、
// 行少底部補空行(版面穩定); 高度耗盡(防禦)回空字串。
func (this *logPanel) View(width, height int) string {
	if height <= 0 {
		return ""
	} // if
