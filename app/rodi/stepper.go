...
// done 緩衝 1: 終局只在最後一拍放行後送出, 不會搶在事件之前到達。
type stepper struct {
...
// 建構與首拍之間讀到的是開局建置前的空白盤面。operator 固定被動觀看(鍵盤 Operator 到 M25 再開參數)。
...
// Next 推進一拍: 放行引擎跑到下一個事件邊界並收回事件(more == true), 引擎停機則收終局成敗(more == false)。
// 同步呼叫且只在 Update 內使用: Bubble Tea 的 Update / View 跑在同一條 goroutine,
// 引擎只在 Next 期間推進 → 「TUI 在看畫面時引擎必停」不需鎖即成立。
func (this *stepper) Next() (eventData cores.EventData, more bool) {
	if this.over {
		return cores.EventData{}, false // 終局後防呆: goroutine 已消滅, 再放行會永久阻塞
	} // if

	this.gate <- struct{}{}

	select {
	case eventData = <-this.event:
		return eventData, true

	case succ := <-this.done:
		this.over = true
		this.succ = succ
		return cores.EventData{}, false
	}
}

// gatePresenter 事件流輸出 port 的交棒半身: Emit 送出事件值(傳值即快照)後停棒等放行——
// 自消費端收走事件起到下一拍放行前, 引擎停在事件邊界(Next 直讀安全窗的另一半)。
type gatePresenter struct {
	event chan<- cores.EventData // 與 stepper.event 同一條
	gate  <-chan struct{}        // 與 stepper.gate 同一條
}

func (this gatePresenter) Emit(eventData cores.EventData) {
