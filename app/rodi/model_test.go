...
func (this *SuiteModel) TestNewModel() {
	target := newModel(nil, 7, 601, "sheetdata", testSheet())
	...
	this.Len(target.comp, 8) // 六區(座位 / 場外 / 行動 / 效果 / 手牌 / 牌堆) + 狀態列 + 鍵位列(M21)
...
func (this *SuiteModel) TestModelInit() {
	this.NotNil(newModel(nil, 1, 601, "sheetdata", testSheet()).Init())
}
...
func (this *SuiteModel) TestModelUpdate() {
	result, cmd := newModel(nil, 1, 601, "sheetdata", testSheet()).Update(eventMsg(cores.EventData{Kind: cores.EventProperty, Round: 2, Attr: "morale", After: 30}))
	...
	_, cmd = newModel(nil, 1, 601, "sheetdata", testSheet()).Update(doneMsg(true))
	this.NotNil(cmd) // 終局行印進 scrollback(成敗常駐顯示歸狀態列階段欄)

	_, cmd = newModel(nil, 1, 601, "sheetdata", testSheet()).Update(doneMsg(false))
	this.NotNil(cmd)

	result, cmd = newModel(nil, 1, 601, "sheetdata", testSheet()).Update(tea.WindowSizeMsg{Width: 120, Height: 40})
	this.Equal(120, result.(model).width)
	this.Nil(cmd)

	_, cmd = newModel(nil, 1, 601, "sheetdata", testSheet()).Update(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune("q")})
	this.NotNil(cmd)
	this.Equal(tea.QuitMsg{}, cmd())

	_, cmd = newModel(nil, 1, 601, "sheetdata", testSheet()).Update(tea.KeyMsg{Type: tea.KeyCtrlC})
	this.NotNil(cmd)
	this.Equal(tea.QuitMsg{}, cmd())

	result, cmd = newModel(nil, 1, 601, "sheetdata", testSheet()).Update(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune("x")})
	this.Equal(0, result.(model).serial)
	this.Nil(cmd)

	result, cmd = newModel(nil, 1, 601, "sheetdata", testSheet()).Update(struct{}{}) // 未知訊息型別不動作
	this.Equal(0, result.(model).serial)
	this.Nil(cmd)
}
