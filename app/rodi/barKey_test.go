...
// (shift+tab 併入 tab 標籤不列示)+ space 模式循環 + n 步進前進 + q 離開 + ctrl+c 逃生(不列示),
// 順序照【營業顯示規格書 | 6、畫面規格 | 6.11】常態終態。
func (this *SuiteBarKey) TestNewBarKey() {
	target := newBarKey()
	this.Require().Len(target.bind, 3)
	this.Require().Len(target.bind[keyModeNormal], 6)
	...
	this.Equal(" ", target.bind[keyModeNormal][2].key)
	this.Equal("n", target.bind[keyModeNormal][3].key)
	this.Equal("q", target.bind[keyModeNormal][4].key)
	this.Equal("ctrl+c", target.bind[keyModeNormal][5].key)
	this.Equal("", target.bind[keyModeNormal][5].label) // 逃生鍵有作用不列示
