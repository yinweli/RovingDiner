...
func (this *SuiteHelp) TestPlaceCard() {
	...
	placeCard(game, cores.ContainerDeck, card) // 進抽牌牌堆: 無事件屬性與觸發
...
// TestPlaceCardEmit 驗證 placeCard 的搬移行收口: 識別碼 >> 去向, 進抽牌牌堆亦發(無事件屬性與觸發仍有搬移行)。
func (this *SuiteHelp) TestPlaceCardEmit() {
	game, record := newGameRecord()
	card := cores.NewCard(game, 101) // 實例編號 1

	placeCard(game, cores.ContainerDeck, card) // 新建直入抽牌牌堆
	this.Require().Len(record.Line, 1)
	this.Equal([]string{"$ 101@#1 >> 抽牌堆"}, record.Line[0])

	removeCard(game, cores.ContainerDeck, card)
	placeCard(game, cores.ContainerHand, card) // 搬移: 抽牌 → 手牌
	this.Require().Len(record.Line, 2)
	this.Equal([]string{"$ 101@#1 >> 手牌"}, record.Line[1])
}
