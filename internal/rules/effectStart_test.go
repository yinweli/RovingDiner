...
func (this *SuiteEffectStart) TestSelectTargets() {
	...
	this.Equal([]cores.Ref{{}}, selectTargets(game, cores.EffectData{TargetKind: cores.TargetNone}, 0, &inherit))
	this.Nil(inherit)

	// 新選顧客 count 2、群組 7:候選 g1/g2（g3 免疫排除）、候選 2 ≤ 2 → 退化全取、設沿用來源
	got := selectTargets(game, cores.EffectData{TargetKind: cores.TargetGuestPick, TargetCount: 2}, 7, &inherit)
	...
	this.Equal([]cores.Ref{cores.NewRefGuest(g1)}, selectTargets(game, cores.EffectData{TargetKind: cores.TargetGuestPick, TargetCount: 1}, 0, &inherit))

	// 隨機顧客 count 1、群組 0:候選 3 > 1 → randSubset 決定性取 1
	this.Len(selectTargets(game, cores.EffectData{TargetKind: cores.TargetGuestRand, TargetCount: 1}, 0, &inherit), 1)

	// 新選手牌 count 1:候選 2 > 1 → PickCard 回前綴 [card 10]
	this.Equal([]cores.Ref{cores.NewRefCard(game.Hand[0])}, selectTargets(game, cores.EffectData{TargetKind: cores.TargetCardPick, TargetCount: 1}, 0, &inherit))

	// 隨機手牌 count 3:候選 2 ≤ 3 → 退化全取
	this.Len(selectTargets(game, cores.EffectData{TargetKind: cores.TargetCardRand, TargetCount: 3}, 0, &inherit), 2)
}
...
func (this *SuiteEffectStart) TestSelectTargetsSame() {
	...
	inherit := []cores.Ref{cores.NewRefGuest(g1)}
	this.Equal([]cores.Ref{cores.NewRefGuest(g1)}, selectTargets(game, cores.EffectData{TargetKind: cores.TargetGuestSame, TargetCount: 1}, 0, &inherit))
	...
	// 沿用手牌:沿用來源有手牌 → 回沿用
	inherit = []cores.Ref{cores.NewRefCard(c1)}
	this.Equal([]cores.Ref{cores.NewRefCard(c1)}, selectTargets(game, cores.EffectData{TargetKind: cores.TargetCardSame, TargetCount: 1}, 0, &inherit))
