...
// 它是規格書【十一、觸發條件】【十二、觸發次數】【十七、命令(RHS 算術式)】
// 共用的運算式 SSOT,語法定義見規格書【二十六、內建函式】【二十七、運算式】。
//
// 與 game 的唯一接縫是 Resolver:expr 只懂語法與運算,遇到「條件對象」
// (morale、self.calm、drawLast.cardID、tableCount('>=',2)…)時呼叫 Resolver 取值。
// game(M2)實作 Resolver,把【二十三、屬性清單】的綁定塞進去。
//
// 評估不拋錯:語意失敗以 Eval 的第二回傳值 ok=false 表達(即【二十七〇8】評估失敗狀態);
