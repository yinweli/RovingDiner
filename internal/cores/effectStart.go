...
func runEffectList(eng *Engine, effectID []int32, skillGroup int32) {
	...
	for _, itor := range effectID {
		...
		for _, self := range selectTargets(eng, meta, skillGroup, &inherit) {
			dispatchEffect(eng, meta, itor, self)
		} // for
	} // for
}
...
func dispatchEffect(eng *Engine, meta effectData, effectID int32, self Ref) {
	...
	defer func() { eng.self = prev }()

	switch meta.Kind {
	case EffectImmed:
		if condPass(eng, meta.Cond) {
			runEffectCommand(eng, meta.Immed, 1) // 立即:條件成立執行立即命令一次（不入佇列）
		} // if

	case EffectTrigger:
		effectStack(eng, self, effectID, 0) // 入佇列;觸發命令留 fireTrigger

	case EffectPersist:
		added := effectStack(eng, self, effectID, 0)
		runEffectCommand(eng, meta.Start, added) // 常駐:啟動命令每增加一層執行一次
	} // switch
}
...
func selectTargets(eng *Engine, meta effectData, skillGroup int32, inherit *[]Ref) []Ref {
	...
	case TargetGuestRand:
		result = setInherit(inherit, pickGuestSelf(eng, skillGroup, meta.TargetCount, true))

	case TargetCardPick:
		result = setInherit(inherit, pickCardSelf(eng, meta.TargetCount, false))

	case TargetCardRand:
		result = setInherit(inherit, pickCardSelf(eng, meta.TargetCount, true))

	case TargetGuestSame:
		if hasGuest(*inherit) {
			result = *inherit // 沿用顧客:繼承前一效果 self（沿用來源不變）
		} else {
			result = setInherit(inherit, pickGuestSelf(eng, skillGroup, meta.TargetCount, false)) // 退化新選顧客
		} // if
...
func cardCandidate(eng *Engine) []*Card {
	return eng.runtime.Hand
}

// selectN 依【營業規格書 | 九、目標數量】退化規則選 count 個:候選 ≤ count → 全取;否則 random 走 randSubset、新選交 pick（玩家）。
func selectN[T any](eng *Engine, candidate []T, count int32, random bool, pick func([]T, int) []T) []T {
	if int32(len(candidate)) <= count {
		return candidate // 退化:候選不足 → 全取
	} // if

	if random {
		return randSubset(eng, candidate, int(count))
	} // if
