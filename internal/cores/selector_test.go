...
func (this *SuiteSelector) TestSelectNone() {
	...
	this.Empty(this.must(game, "none", nil))
}

func (this *SuiteSelector) TestSelectSelf() {
	this.Equal([]InstanceID{1}, this.must(this.inject(NewGame(0, nil, nil, nil, nil), &Ref{card: &Card{instanceID: 1}}), "self", nil))   // 綁定卡牌
	this.Equal([]InstanceID{2}, this.must(this.inject(NewGame(0, nil, nil, nil, nil), &Ref{guest: &Guest{instanceID: 2}}), "self", nil)) // 綁定顧客
	this.Empty(this.must(this.inject(NewGame(0, nil, nil, nil, nil), &Ref{}), "self", nil))                                              // 綁定空物件
	this.Empty(this.must(this.inject(NewGame(0, nil, nil, nil, nil), nil), "self", nil))                                                 // 未綁定
}

func (this *SuiteSelector) TestSelectSelfNearSame() {
	game, g1 := seatGuestGame()

	near := this.inject(game, &Ref{guest: g1})
	...
	card := this.inject(game, &Ref{card: &Card{instanceID: 9}})
	...
	roam := this.inject(game, &Ref{guest: &Guest{instanceID: 99, seatID: 0}})
...
func (this *SuiteSelector) TestSelectEventSingle() {
	...
	for name, want := range map[string]InstanceID{
		...
		this.Equal([]InstanceID{want}, this.must(game, name, nil), name)
	} // for

	// 無事件來源(欄位 nil)→ 空集合(cardOne / guestOne 的 nil 分支)
	empty := this.inject(NewGame(0, nil, nil, nil, nil), nil)
	this.Empty(this.must(empty, "drawLast", nil))
	this.Empty(this.must(empty, "seatLast", nil))
}

func (this *SuiteSelector) TestSelectGuestAll() {
	game, _ := seatGuestGame()
	this.Equal([]InstanceID{11, 12, 13}, this.must(this.inject(game, nil), "guestAll", nil)) // 依座位編號升序
}
...
func (this *SuiteSelector) TestSelectGuestPick() {
	...
	this.Equal([]InstanceID{11, 12}, this.must(game, "guestPick", nums(2)))     // 候選 3 > 2 → Operator 取前 2
	this.Equal([]InstanceID{11, 12, 13}, this.must(game, "guestPick", nums(5))) // 候選 3 <= 5 → 退化取全部(不彈介面)
...
func (this *SuiteSelector) TestSelectGuestRand() {
	...
	this.Equal([]InstanceID{11, 12, 13}, this.must(game, "guestRand", nums(3))) // 候選 3 <= 3 → 退化取全部
	this.Equal([]InstanceID{11}, this.must(game, "guestRand", nums(1)))         // 候選 3 > 1 → 隨機取 1(恆等洗牌取首位)
	this.Empty(this.must(game, "guestRand", nums(0)))                           // N = 0 → 空集合
	this.Equal([]InstanceID{11, 12}, this.must(game, "guestRand", nums(-1)))    // N < 0 → 保留 1、取出其餘 2
...
func (this *SuiteSelector) TestSelectGuestWait() {
	game := NewGame(0, nil, nil, nil, nil)
	game.Wait = []*Guest{{instanceID: 21}, {instanceID: 22}, {instanceID: 23}}
	injectPort(game)

	this.Equal([]InstanceID{21, 22}, this.must(game, "guestWait", nums(2)))     // 取隊首 2 位
	this.Equal([]InstanceID{21, 22, 23}, this.must(game, "guestWait", nums(5))) // N >= 長度 → 取全部
...
func (this *SuiteSelector) TestSelectNearSame() {
	...
	this.Equal([]InstanceID{13}, this.must(game, "nearPick", nil))     // Operator 選 g1(座1)→ 鄰桌座3 = g3
	this.Equal([]InstanceID{13}, this.must(game, "nearRand", nil))     // Rander 選首位 g1 → 鄰桌 g3
	this.Equal([]InstanceID{11, 12}, this.must(game, "samePick", nil)) // g1 同桌 = g1,g2
	this.Equal([]InstanceID{11, 12}, this.must(game, "sameRand", nil))

	// 空座位:無錨點可選 → 空集合(pickGuestOne / randGuestOne 的空候選分支)
	noSeat := this.inject(NewGame(0, nil, nil, nil, nil), nil)
	...
	one := NewGame(0, nil, nil, nil, nil)
	one.Seat[2] = &Guest{instanceID: 12, seatID: 2}
	this.Equal([]InstanceID{12}, this.must(this.inject(one, nil), "samePick", nil))

	// Operator 回空(防禦):候選 > 1 但選不出 → 空集合(pickGuestOne 的 chosen 空分支)
	game.operator = fakeOperator{emptyPick: true}
	this.Empty(this.must(game, "nearPick", nil))
}

func (this *SuiteSelector) TestSelectContainerAll() {
	game := NewGame(0, nil, nil, nil, nil)
	game.Hand = []*Card{{instanceID: 1, cardID: 101}, {instanceID: 2, cardID: 102}}
	game.Deck = []*Card{{instanceID: 3, cardID: 101}}
	game.Drop = []*Card{{instanceID: 4, cardID: 103}}
	game.Exile = []*Card{{instanceID: 5, cardID: 104}}
	injectPort(game)

	this.Equal([]InstanceID{1, 2}, this.must(game, "handAll", nums(0))) // 編號 0 → 取全部
	this.Equal([]InstanceID{1}, this.must(game, "handAll", nums(101)))  // 編號 101 → 篩出
	this.Empty(this.must(game, "handAll", nums(999)))                   // 無此編號 → 空集合
	this.Empty(this.must(game, "handAll", nil))                         // 缺參數 → 空集合

	this.Equal([]InstanceID{3}, this.must(game, "deckAll", nums(0))) // 各容器 All 變體一一驗證
	this.Equal([]InstanceID{4}, this.must(game, "dropAll", nums(0)))
	this.Equal([]InstanceID{5}, this.must(game, "exileAll", nums(0)))
}

func (this *SuiteSelector) TestSelectContainerPick() {
	game := NewGame(0, nil, nil, nil, nil)
	game.Hand = []*Card{{instanceID: 1}, {instanceID: 2}, {instanceID: 3}}
	game.Deck = []*Card{{instanceID: 4}}
	game.Drop = []*Card{{instanceID: 5}}
	game.Exile = []*Card{{instanceID: 6}}
	injectPort(game)

	this.Equal([]InstanceID{1, 2}, this.must(game, "handPick", nums(2, 0)))    // 候選 3 > 2 → Operator 取前 2
	this.Equal([]InstanceID{1, 2, 3}, this.must(game, "handPick", nums(5, 0))) // 候選 <= N → 退化取全部
	this.Empty(this.must(game, "handPick", nums(0, 0)))                        // N = 0 → 空集合
	this.Empty(this.must(game, "handPick", nums(1)))                           // 參數不足(缺卡牌編號)→ 空集合

	this.Equal([]InstanceID{4}, this.must(game, "deckPick", nums(1, 0))) // 各容器 Pick 變體一一驗證
	this.Equal([]InstanceID{5}, this.must(game, "dropPick", nums(1, 0)))
	this.Equal([]InstanceID{6}, this.must(game, "exilePick", nums(1, 0)))
}
...
func (this *SuiteSelector) TestSelectContainerRand() {
	...
	this.Equal([]InstanceID{1, 2}, this.must(game, "handRand", nums(2, 0))) // 候選 3 > 2 → 隨機取 2(恆等洗牌取前綴)
	this.Empty(this.must(game, "handRand", nums(1)))                        // 參數不足 → 空集合

	this.Equal([]InstanceID{4}, this.must(game, "deckRand", nums(1, 0))) // 各容器 Rand 變體一一驗證
	this.Equal([]InstanceID{5}, this.must(game, "dropRand", nums(1, 0)))
	this.Equal([]InstanceID{6}, this.must(game, "exileRand", nums(1, 0)))
}

func (this *SuiteSelector) TestSelectDeckTop() {
	// deck >= N:取頂端(前端)N,不洗牌
	deckGe := NewGame(0, nil, nil, nil, nil)
	deckGe.Deck = cards(1, 2, 3)
	deckGe.Drop = cards(4)
	this.Equal([]InstanceID{1, 2}, this.must(this.inject(deckGe, nil), "deckTop", nums(2)))
	this.Len(deckGe.Drop, 1) // 牌堆足夠 → 未洗牌移動

	// deck < N 且 deck+drop >= N:洗棄牌移至牌堆底端再取頂端 N
	deckShort := NewGame(0, nil, nil, nil, nil)
	deckShort.Deck = cards(1)
	deckShort.Drop = cards(4, 5)
	this.Equal([]InstanceID{1, 4}, this.must(this.inject(deckShort, nil), "deckTop", nums(2))) // 原牌堆 1 在頂、洗入的 4 接其後
	this.Empty(deckShort.Drop)                                                                 // 棄牌已全數移入

	// deck+drop < N:洗棄牌移入後取全部
	deckTiny := NewGame(0, nil, nil, nil, nil)
	deckTiny.Deck = cards(1)
	deckTiny.Drop = cards(4)
	this.Equal([]InstanceID{1, 4}, this.must(this.inject(deckTiny, nil), "deckTop", nums(5)))

	// deck < N 但棄牌為空:不洗牌、取全部
	deckNoDrop := NewGame(0, nil, nil, nil, nil)
	deckNoDrop.Deck = cards(1)
	this.Equal([]InstanceID{1}, this.must(this.inject(deckNoDrop, nil), "deckTop", nums(5)))

	// N <= 0 / 缺參數 → 空集合
	this.Empty(this.must(this.inject(deckGe, nil), "deckTop", nums(0)))
	this.Empty(this.must(this.inject(deckGe, nil), "deckTop", nil))
}

func (this *SuiteSelector) TestSelectDropTop() {
	game := NewGame(0, nil, nil, nil, nil)
	game.Drop = cards(1, 2, 3)
	injectPort(game)

	this.Equal([]InstanceID{1, 2}, this.must(game, "dropTop", nums(2)))    // 取頂端 2
	this.Equal([]InstanceID{1, 2, 3}, this.must(game, "dropTop", nums(9))) // 不足 → 取全部
	this.Empty(this.must(game, "dropTop", nil))                            // 缺參數 → 空集合
}

// === 測試輔助(置尾) ===

// inject 注入測試替身(injectPort)並綁定 self;回傳同一 Game 供鏈式使用。
func (this *SuiteSelector) inject(game *Game, self *Ref) *Game {
	injectPort(game)
	game.self = self
	return game
}
...
func (this *SuiteSelector) must(game *Game, name string, arg []exprs.Value) (result []InstanceID) {
	result, ok := game.selectObject(name, arg)
	this.Require().True(ok, name)
	return result
}

// seatGuestGame 組裝 3 位在座顧客(座1=g1=11 / 座2=12 / 座3=13;桌1=座1,2、桌2=座3,對齊 buildSheet);回傳 g1 供 self 綁定。
func seatGuestGame() (game *Game, g1 *Guest) {
	game = NewGame(0, nil, nil, nil, nil)
	g1 = &Guest{instanceID: 11, seatID: 1}
	game.Seat[1] = g1
	game.Seat[2] = &Guest{instanceID: 12, seatID: 2}
