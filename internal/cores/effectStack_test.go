...
func (this *SuiteEffectStack) TestEffectStack() {
	...
	this.Equal(int32(2), effectStack(eng, Self{Guest: guest}, 801, 0)) // 增量 2 ≤ 上限 3
	this.Require().Len(runtime.Effect, 1)
	this.Equal(int32(2), runtime.Effect[0].Stack)
	this.Equal(int32(6), runtime.Effect[0].Expire) // 5 + 2 − 1

	// 既有同份 → 疊層、min(2+2, 3) = 3、回實際增加 1
	this.Equal(int32(1), effectStack(eng, Self{Guest: guest}, 801, 0)) // 2 → 3（達上限,只 +1）
	this.Len(runtime.Effect, 1)                                        // 仍一筆
	this.Equal(int32(3), runtime.Effect[0].Stack)

	// 已達上限再疊 → 回 0
	this.Equal(int32(0), effectStack(eng, Self{Guest: guest}, 801, 0))

	// override N:802 無上限、0/1→1 被 override 取代
	this.Equal(int32(5), effectStack(eng, Self{Guest: guest}, 802, 5)) // override 5
	this.Require().Len(runtime.Effect, 2)
	this.Equal(int32(5), runtime.Effect[1].Stack)
	this.Equal(int32(0), runtime.Effect[1].Expire) // RunRound 0 → 整場
}

func (this *SuiteEffectStack) TestEffectStackImmune() {
	guest := &Guest{InstanceID: 11, EffectImmune: map[int32]int32{5: 1}} // 免疫群組 5
	runtime := NewRuntime(0)
	eng := this.engine(runtime, map[int32]effectData{
		801: {Group: 5, Stack: 1},
	})

	this.Equal(int32(0), effectStack(eng, Self{Guest: guest}, 801, 0)) // 顧客免疫 → 0、不入列
	this.Empty(runtime.Effect)

	// 卡牌 self 不受免疫閘門（免疫為顧客限定）
	this.Equal(int32(1), effectStack(eng, Self{Card: &Card{InstanceID: 12}}, 801, 0))
	this.Len(runtime.Effect, 1)

	this.Equal(int32(0), effectStack(eng, Self{Guest: guest}, 999, 0)) // 查無編譯資料 → no-op
}
...
func (this *SuiteEffectStack) TestSameSelf() {
	g1 := &Guest{InstanceID: 1}
	g2 := &Guest{InstanceID: 2}
	c1 := &Card{InstanceID: 1}

	this.True(sameSelf(Self{Guest: g1}, Self{Guest: g1}))  // 同顧客
	this.False(sameSelf(Self{Guest: g1}, Self{Guest: g2})) // 不同顧客
	this.True(sameSelf(Self{Card: c1}, Self{Card: c1}))    // 同卡牌
	this.True(sameSelf(Self{}, Self{}))                    // 皆無目標
	this.False(sameSelf(Self{Guest: g1}, Self{Card: c1}))  // 型別不符
	this.False(sameSelf(Self{Guest: g1}, Self{}))          // 顧客 vs 無目標
}
