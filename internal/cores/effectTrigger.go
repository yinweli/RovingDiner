...
func fireTrigger(eng *Engine, timing TriggerKind) {
	fire := []*Effect{}
	...
	effectSort(eng, fire) // 快照排序;不擾動 runtime.Effect（順序無關）
...
func fireOne(eng *Engine, effect *Effect) {
	...
	if condPass(eng, meta.Cond) == false {
		return // 觸發條件不成立（【十一】評估失敗即不成立）
	} // if

	count, ok := triggerCount(eng, meta.Count)
...
func condPass(eng *Engine, cond *exprs.Expr) bool {
	if cond == nil {
		return true
	} // if

	result, ok := cond.Eval(eng.env())
...
func triggerCount(eng *Engine, count *exprs.Expr) (n int32, ok bool) {
	if count == nil {
		return 1, true
	} // if

	result, valid := count.Eval(eng.env())
...
func runEffectCommand(eng *Engine, command EffectCommand, times int32) {
	if command == nil {
		return
	} // if
