...
func commandCardRun(game *Game, target []InstanceID, arg []exprs.Value) {
	...
	for _, itor := range target {
		...
		if toDrop {
			if _, now, found := game.locateCard(itor); found && now != ContainerDrop {
				removeCard(game, now, card)
				placeCard(game, ContainerDrop, card) // 進棄牌牌堆（設 dropLast 等 + M11 cardDrop 觸發）
			} // if
		} // if

		fireTrigger(game, TriggerCardPlay) // 玩家出牌觸發
	} // for
}

// === 變身 *Morph（對象；抽獎群組編號）===

func commandHandMorph(game *Game, target []InstanceID, arg []exprs.Value) {
	morph(game, target, arg, ContainerHand)
}

func commandDeckMorph(game *Game, target []InstanceID, arg []exprs.Value) {
	morph(game, target, arg, ContainerDeck)
}

func commandDropMorph(game *Game, target []InstanceID, arg []exprs.Value) {
	morph(game, target, arg, ContainerDrop)
}

func commandExileMorph(game *Game, target []InstanceID, arg []exprs.Value) {
	morph(game, target, arg, ContainerExile)
}

// morph 變身處理流程：抽獎取新 cardID、就地換 cardID + 重分配實例編號 + 依新卡資料載入部分欄位、設變身事件；
// 位置不符 / 群組總權重 0 該項 no-op。source 留原牌堆位置、不觸發 cardDraw / Drop / Exile。
func morph(game *Game, target []InstanceID, arg []exprs.Value, where ContainerKind) {
	group, ok := argInt(arg)
	...
	for _, itor := range target {
		...
		newID, rolled := rollCard(game, group)
		...
		fireTrigger(game, TriggerCardMorph) // 卡牌變身觸發
	} // for
}

// commandCardify 卡牌化顧客（cardify 處理流程）：將座位顧客移入卡牌化列表、凍結、實例化對應卡牌（不棄 + 綁來源）加入手牌。
// 顧客不在座位列表 / 卡牌資料不存在 → 該項 no-op。參數：卡牌編號。
func commandCardify(game *Game, target []InstanceID, arg []exprs.Value) {
...
func commandGuestExit(game *Game, target []InstanceID, arg []exprs.Value) {
...
func commandGuestReturn(game *Game, target []InstanceID, arg []exprs.Value) {
	for _, itor := range target {
		guest, where, found := game.locateGuest(itor)
		...
		if ok == false {
			guestExitOne(game, guest, ContainerRoam, false, true) // 3. 剩餘座位 = 0 → 離場
			continue
		} // if
...
// 參數：無。
func commandGuestRoam(game *Game, target []InstanceID, arg []exprs.Value) {
	for _, itor := range target {
		guest, where, found := game.locateGuest(itor)
...
func commandGuestSeat(game *Game, target []InstanceID, arg []exprs.Value) {
	if len(game.Wait) == 0 {
		return // 排隊佇列空 → no-op
	} // if
	...
	fireTrigger(game, TriggerGuestSeat) // 顧客入座觸發
}

// === 流程輔助 ===

// guestExitOne 對單一顧客執行 guestExit 處理流程（供 commandGuestExit 與 guestReturn 無座位分支共用）。
func guestExitOne(game *Game, guest *Guest, where ContainerKind, giveScore, dropMorale bool) {
	game.EventExit(guest)             // 1. 離場事件（離場座位取自當下 seatID,故在容器移除前）
	fireTrigger(game, TriggerExitAny) // 2. 顧客離場時機

	if giveScore {
		fireTrigger(game, TriggerExitSate) // 3. 飽食離場時機（提供滿意值前）
	} // if

	if dropMorale {
		fireTrigger(game, TriggerExitCalm) // 4. 生氣離場時機（扣士氣前）
	} // if

	removeGuestContainer(game, where, guest) // 5. 自所在容器移除
	fireTrigger(game, TriggerExitDone)       // 6. 顧客離場後時機
...
func removeGuestContainer(game *Game, where ContainerKind, guest *Guest) {
	switch where {
	case ContainerSeat:
		game.Seat.Remove(guest)
...
func randomEmptySeat(game *Game) (seatID int32, ok bool) {
	id := game.data.Seat.Keys()
	...
	return empty[game.rander.Intn(len(empty))], true
}
