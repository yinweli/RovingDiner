...
func (this *SuiteEffectStart) TestSelectTargets() {
	...
	this.Equal([]Ref{{}}, selectTargets(eng, effectData{TargetKind: TargetNone}, 0, &inherit))
	this.Nil(inherit)
	...
	this.Equal([]Ref{NewRefGuest(g1)}, selectTargets(eng, effectData{TargetKind: TargetGuestPick, TargetCount: 1}, 0, &inherit))
	...
	this.Equal([]Ref{NewRefCard(runtime.Hand[0])}, selectTargets(eng, effectData{TargetKind: TargetCardPick, TargetCount: 1}, 0, &inherit))
...
func (this *SuiteEffectStart) TestSelectTargetsSame() {
	...
	inherit := []Ref{NewRefGuest(g1)}
	this.Equal([]Ref{NewRefGuest(g1)}, selectTargets(eng, effectData{TargetKind: TargetGuestSame, TargetCount: 1}, 0, &inherit))
	...
	// 沿用手牌:沿用來源有手牌 → 回沿用
	inherit = []Ref{NewRefCard(c1)}
	this.Equal([]Ref{NewRefCard(c1)}, selectTargets(eng, effectData{TargetKind: TargetCardSame, TargetCount: 1}, 0, &inherit))
