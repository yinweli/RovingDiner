...
4. **可移植子集明確**：`expr` + `game` + instance 模型 + `defines` 可移植到 C#，`features`、`app/tui` 為 Go-only 基礎設施。
...
專案處於早期階段。目前包含 Sheeter 遊戲資料管線（`gamedata/` → `sheet/` + `sheetdata/`）與 `doc/` 下的設計文件；營業核心邏輯（營業迴圈、模組、執行期實例）尚未實作。
